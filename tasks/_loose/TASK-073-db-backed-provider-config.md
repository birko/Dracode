...
3. **Secret/cipher seam**: register `ISecretProvider` (Phase-1 default = config-backed master-key provider; dev via user-secrets) + a thin `ProviderKeyCipher` over `AesEncryptionProvider` that encrypts/decrypts using the `ISecretProvider`-resolved master key. Never log plaintext.
4. **Service refactor**: `ProviderConfigurationService` backing store → repository + in-memory cache; add `InitializeAsync()` (load cache; one-time import when the provider table is empty: appsettings `Providers` + `user-settings.json` + env keys → DB, keys encrypted). Keep every public method signature; replace the env-var key read with `ProviderKeyCipher.Decrypt(row.ApiKeyCiphertext)`; model resolution reads `ProviderModelEntity` (default + per-agent selection). Writes update DB **and** cache (runtime reload).
