...
| D14 | Issue service-account tokens via a consumer-side issuer (reusing the existing `/register` for client registration), not by forking the framework OAuth token endpoint | approved | The stock client-credentials grant hardcodes `sub = client.ClientId` and space-joins scopes (which the comma-splitting `ClaimsCurrentUser` can't enforce). A thin consumer issuer stamps `sub = "service:<name>"` + comma-joined permission scopes via the existing `ITokenProvider`, leaving `Birko.Security.OAuth.Server` generic. Refines D4 | 2026-06-22 | human | TASK-035 |
| D15 | Service-account scopes use the existing permission-constant vocabulary (`manage_projects`, `execute_agents`, `view_own`, …), not a new `resource:action` scheme | approved | Endpoints enforce against permission constants via `PermissionEndpointFilter`; reusing that vocabulary keeps one enforcement model across humans and services and avoids a translation layer. The criteria's `projects:write`/`runs:write` are treated as illustrative. Refines D4 | 2026-06-22 | human | TASK-035 |
