...
namespace DraCode.KoboldTown.Services
{
    public class WyvernService
    {
        ...
        public WyvernService(ILogger<WyvernService> logger, IConfiguration configuration)
        {
            _logger = logger;
            _taskTracker = new TaskTracker();
            _provider = configuration["wyvern:Provider"] ?? "openai";
            _config = new Dictionary<string, string>
            {
                ["apiKey"] = configuration["wyvern:ApiKey"] ?? "",
                ["model"] = configuration["wyvern:Model"] ?? "gpt-4o"
            };
        }
