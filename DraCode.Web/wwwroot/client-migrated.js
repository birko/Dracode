...
export class DraCodeClient {
    constructor() {
        ...
        this.elements = {
            ...
            disconnectBtn: this.getElement('disconnectBtn'),
            listBtn: this.getElement('listBtn'),
            providersSection: this.getElement('providersSection'),
            providersGrid: this.getElement('providersGrid'),
            wsUrl: this.getElement('wsUrl'),
            ...
            connectionTabName: this.getElement('connectionTabName'),
            connectionWorkingDir: this.getElement('connectionWorkingDir'),
            connectionModalConnect: this.getElement('connectionModalConnect'),
            connectionModalCancel: this.getElement('connectionModalCancel'),
            ...
            generalModalInput: this.getElement('generalModalInput'),
            ...
            generalModalConfirm: this.getElement('generalModalConfirm'),
            generalModalCancel: this.getElement('generalModalCancel')
        };
    ...
    setupEventListeners() {
        ...
        const filterConfigured = document.getElementById('filterConfigured');
        const filterAll = document.getElementById('filterAll');
        const filterNotConfigured = document.getElementById('filterNotConfigured');
        if (filterConfigured) {
    ...
    handleServerMessage(event) {
        ...
        try {
            ...
            if (agentId && this.agents.has(agentId)) {
                this.logToAgent(agentId, this.formatResponse(response), response.Status === 'error' ? 'error' : 'success');
                ...
                this.logToConsole(this.formatResponse(response), response.Status === 'error' ? 'error' : 'success');
            }
        }
        catch (error) {
    ...
    formatResponse(response) {
        ...
        if (response.Data)
            msg += `<br><pre>${this.escapeHtml(response.Data)}</pre>`;
        if (response.Error)
            msg += `<br><span class="error">${this.escapeHtml(response.Error)}</span>`;
        return msg;
    }
    /**
        ...
        ['serverConnection', 'providersContent'].forEach(sectionId => {
            ...
            if (content && icon && header && !content.classList.contains('collapsed')) {
                ...
                if (sectionId === 'providersContent') {
                    ...
                    if (this.availableProviders.length > 0) {
                        this.updateProviderCountBadge(header, this.availableProviders.length);
                    }
                }
            }
        });
     ...
     */
    updateProviderCountBadge(header, count) {
     ...
     */
    async handlePromptMessage(response) {
        const agentId = response.AgentId;
     ...
     */
    async connectToProvider(providerName, agentId) {
        if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
     ...
     */
    connectToProviderWithName(providerName, agentId, tabName, workingDir) {
     ...
     */
    async connectManualProvider(provider, apiKey, model, workingDir) {
        if (!provider || !apiKey) {
     ...
     */
    createAgentTab(agentId, displayName, providerName) {
        ...
        const taskTextarea = content.querySelector(`#task-${agentId}`);
        if (sendBtn) {
     ...
     */
    async sendTaskToAgent(agentId) {
        const taskInput = document.getElementById(`task-${agentId}`);
        if (!taskInput)
            return;
     ...
     */
    logToAgent(agentId, message, level = 'info') {
        const logElement = document.getElementById(`log-${agentId}`);
        if (!logElement)
            return;
